...
NormalizedLoopBandwidths = [0.1,0.15];
%% Generate symbols
...
%f1 = figure(1);
subplot(2,1,1)
plot(phErrs(:,1));%hold on;
xlim([0 1000]);grid on;xlabel('Time');ylabel('PED Error');
ylim([-1.1 1.1]);
title(['Error rate for damping factor = ',num2str(DampingFactor),...
    ' and loop bandwidth = ', num2str(NormalizedLoopBandwidths(1))])
subplot(2,1,2)
plot(phErrs(:,2));%hold off;
xlim([0 1000]);grid on;xlabel('Time');ylabel('PED Error');
ylim([-1.1 1.1]);
title(['Error rate for damping factor = ',num2str(DampingFactor),...
    ' and loop bandwidth = ', num2str(NormalizedLoopBandwidths(2))])