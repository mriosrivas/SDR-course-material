...
% Define EVMdb as anonymous function
evmdb = @(x) 20*log10(x/100);
...
disp(['EVMdb: ',num2str(evmdb(evm_rms)),'dB']);


% Determine EVM RMS using comm.EVM
evm = comm.EVM('Normalization', 'Average reference signal power');
rmsEVM = evm(output, est);
disp(['EVM (Toolbox): ',num2str(rmsEVM),'%']);
disp(['EVMdb (Toolbox): ',num2str(evmdb(rmsEVM)),'dB']);
